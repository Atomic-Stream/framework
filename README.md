...
Before executing any scripts, the organization must establish the human container[cite: 1].
* Read the Atomic Stream Framework Guide.
* Formally appoint the Stream Orchestrator, System Architect, and Platform Engineer[cite: 1].
...
* Configure the Scribe service to route agent output directly to the Stream's system of record[cite: 1].

## Phase 4: Pair Instantiation
Pairs execute work in isolated environments by dynamically reading from this repository.
* The Stream Orchestrator assigns an Intent to a specific Pair[cite: 1].
* Pilot Developers trigger local worker agents using the provided Agent Brief templates[cite: 1].
* Agents load rules from the `main` branch, execute the work, and halt if boundaries are crossed[cite: 1].
